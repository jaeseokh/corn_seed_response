...
# Prepare ppi data and calculate annual average
ppi_dat <- ppi_dat %>%
...
# Set 1996 PPI as baseline
wpu_1996 <- ppi_dat %>% filter(year == 1996) %>% pull(wpu_annual)

# Filter corn data
corn_sel <- corn_ers %>%
  filter(Item %in% c("Primary product, grain","Fertilizer  ", "Seed", "Yield"),
         Region == "U.S. total") %>%
  left_join(ppi_dat, by = c("Year" = "year")) %>%
  mutate(
    wpu_normalized = wpu_annual / wpu_1996,
    Value_adjusted = Value / wpu_normalized
  ) %>%
  drop_na()

saveRDS(corn_sel, here("Data", "Processed","usda_ers_corn_cost_return.rds"))
