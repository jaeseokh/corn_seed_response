...
info_tb_resp <- info_tb_binded[!is.na(resp_type)]
info_tb_resp <- info_tb_resp[, resp_type := factor(resp_type, levels = c("A1", "A2", "B1", "B2", "C"))]
...
x_min_prec <- min(info_tb_resp$prcp_t, na.rm = TRUE)
x_max_prec <- max(info_tb_resp$prcp_t, na.rm = TRUE)
y_min_prec <- min(info_tb_resp$eosr, info_tb_resp$sqsr)
y_max_prec <- max(info_tb_resp$eosr,info_tb_resp$sqsr)
...
x_min_prcp30 <- min(info_tb_resp$prcp_30, na.rm = TRUE)
x_max_prcp30 <- max(info_tb_resp$prcp_30, na.rm = TRUE)
y_min_prcp30 <- min(info_tb_resp$eosr, info_tb_resp$sqsr)
y_max_prcp30 <- max(info_tb_resp$eosr, info_tb_resp$sqsr)
...
eosr_sqsr_prec30 <- ggplot(info_tb_resp) +
  ...
  xlim(200,1000) +
  geom_segment(aes(x = x_min_prcp30, y = y_min_prcp30, xend = x_max_prcp30, yend = y_max_prcp30), color = "black", size = 0.2) +  # Add diagonal line
  labs(  y = NULL,
         x = "Precipitation \n (30 year average)") +
  theme(
...
eosr_sqsr_weather <- (eosr_sqsr_prec | eosr_sqsr_prec30) + # Collect legends from both plots
  theme(
...
```{r figure4 - Estimated Profit and Yield response by type  , cache = T, results = "hide"}
...
info_tb_b2$ffy_id[12]
#ffy_id = "10_1_2019"

eval_type_b2 <-readRDS(here("Data", "processed", "Analysis_results","10_1_2019_eval_tb.rds"))
info_b2_chosen <-info_tb_b2[ffy_id == "10_1_2019",]
